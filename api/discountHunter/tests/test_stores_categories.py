...
class TestCategories(TestCase):
    ...
    def test_find_new_lidl_promotions_categories(self):
        self.assertEqual(len(get_lidl_categories_urls()) > 0, True)

    def test_find_new_lidl_categories_products_urls(self):
        self.assertEqual(len(get_lidl_category_products_url(get_lidl_categories_urls()[0])) > 0, True)
    ...
    def test_find_new_kaufland_categories_urls(self):
        self.assertEqual(len(get_kaufland_categories_urls()) > 0, True)

    def test_find_new_kaufland_categories_products_urls(self):
        self.assertEqual(len(get_kaufland_category_products_url(get_kaufland_categories_urls()[0])) > 0, True)
